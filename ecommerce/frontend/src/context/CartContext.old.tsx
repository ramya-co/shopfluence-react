...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addItem = async (product: any, quantity: number = 1, clickEvent?: MouseEvent) => {
    ...
    if (clickEvent && clickEvent.target instanceof HTMLElement) {
      ...
      const productContainer = button?.closest('.bg-white, .product-card, [class*="product"]') || document;
      
      // Search for price text in various possible selectors
      const priceSelectors = [
        '.text-3xl.font-bold', // ProductDetail main price
        '.text-xl.font-bold',  // ProductCard price
        '.font-bold.text-orange-500', // Various price classes
        '[class*="price"]',    // Any element with "price" in class name
        'span[class*="font-bold"]' // Bold spans that might contain price
      ];
      
      let displayedPrice = null;
      
      for (const selector of priceSelectors) {
        const priceElement = productContainer.querySelector(selector);
        if (priceElement) {
          const priceText = priceElement.textContent || '';
          const priceMatch = priceText.match(/\$?(\d+\.?\d*)/);
          if (priceMatch) {
            displayedPrice = parseFloat(priceMatch[1]);
            break;
          }
        }
      }
      
      // If we found a displayed price and it differs from the original, it's manipulation
      ...
      // Also check for data-price attribute as backup
      if (button && button.hasAttribute('data-price')) {
        const manipulatedPrice = parseFloat(button.getAttribute('data-price') || '0');
